Large list is from https://www.nuclearwarmap.com/targetlist.html

Small list is from https://worldpopulationreview.com/state-rankings/nuclear-targets-by-state

WSEG-10 report from https://apps.dtic.mil/sti/tr/pdf/AD0261752.pdf

Topographic data from https://portal.opentopography.org/raster?opentopoID=OTSRTM.122019.4326.1

License applies to all content except topographic tiles and PDF

Note: `build_terrain.py` now updates `FALLOUT_WSEG10.c` automatically to match the set of locations found in `prepatory_CSVs/MASTER.csv` (so the C menu/sizes stay in sync with your CSV).